...
async function findArtist(slug: string) {
  const rows = await query<ArtistProfile>("artist_profiles", { slug: `eq.${slug}`, is_published: "eq.true", select: "id,artist_name,slug,tagline,bio,image_path,banner_path,accent_color,spotify_url,youtube_url,suno_url,tiktok_url,facebook_url" });
  return rows[0] ?? null;
}
...
export default async function ArtistPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const profile = artist ?? (demo ? {
    ...
    image_path: null, banner_path: null, accent_color: "#d9ff3f", spotify_url: null, youtube_url: null, suno_url: null, tiktok_url: null, facebook_url: null
  ...
  const videos = profile.id === "demo" ? [] : await query<ChannelVideo>("artist_videos", { artist_profile_id: `eq.${profile.id}`, select: "id,youtube_id,youtube_url,title", order: "created_at.desc" });
  ...
  return <main className="shell page channel-page">
    ...
    <article className="channel">
      ...
      <div className="channel-content"><p className="bio">{profile.bio || "Dieses Profil wird gerade aufgebaut."}</p>{links.length > 0 && <div className="links">{links.map(([label, url]) => <a key={label} href={url} target="_blank" rel="noreferrer">{label} ↗</a>)}</div>}<VideoShelf videos={videos} /></div>
    </article>
  </main>;
}
