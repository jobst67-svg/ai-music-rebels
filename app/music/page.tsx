...
export const metadata: Metadata = {
  ...
  description: "Durchsuche AI Music Rebels nach Songs, Künstlern und Genres, höre verfügbare Tracks und stelle lokale Playlists zusammen."
};
...
export default async function MusicPage() {
  const profiles = await query<ProfileRow>("artist_profiles", {
    is_published: "eq.true",
    select: "id,slug,artist_name,genre_primary,genre_secondary"
  });
  ...
  const discoverTracks: DiscoverTrack[] = tracks.flatMap((track) => {
    ...
    return [{
      ...
      artist_name: profile.artist_name || profile.slug,
      artist_slug: profile.slug,
      genre_primary: profile.genre_primary,
      genre_secondary: profile.genre_secondary
    }];
  });

  return <main className="shell page">
    <ProfileNav variant="standard" />
    <MusicBrowser tracks={discoverTracks} />
  </main>;
}
