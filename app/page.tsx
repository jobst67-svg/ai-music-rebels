...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    supabase.from("artist_profiles").select("id,slug,artist_name,image_path,tagline").eq("is_published",true).order("artist_name",{ascending:true}).limit(10)
    ...
    <main className={styles.page}>
      <div className={styles.shell}>
        ...
        <section id="rebels" className={styles.rebelsSection}>
          <div className={styles.sectionHead}><div><h2>{english ? "MEET THE " : "TRIFF DIE "}<em>REBELS</em></h2><p>{english ? "Discover AI artists from around the world." : "Entdecke AI Artists aus aller Welt."}</p></div><div className={styles.arrows}><button type="button" onClick={() => rail.current?.scrollBy({left:-520,behavior:"smooth"})}>‹</button><button type="button" onClick={() => rail.current?.scrollBy({left:520,behavior:"smooth"})}>›</button></div></div>
          <div className={styles.rail} ref={rail}>
            ...
            {rebels.length === 0 && <div className={styles.emptyCard}><span>⚡</span><strong>{english ? "The first Rebels are coming." : "Die ersten Rebels kommen."}</strong><small>{english ? "Your profile can be one of them." : "Dein Profil kann dazugehören."}</small></div>}
